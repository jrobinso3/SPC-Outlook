...
export const ThemeManager = {
    ...
    getAlertStyle(feature) {
        ...
        if (event.includes('Tornado Warning')) {
            ...
            if (isEmergency) {
                color = '#ff00ff'; weight = 4; fillOpacity = 0.55;
            } else if (isObserved || isPDS) {
                color = '#8b0000'; weight = 3.5; fillOpacity = 0.45;
            } else {
                color = '#ff0000'; weight = 2.5; fillOpacity = 0.35;
            }
        } else if (event.includes('Tornado Watch')) {
            color = '#ffff00'; weight = 1.5; fillOpacity = 0.25;
        } else if (event.includes('Severe Thunderstorm Warning')) {
            const isDestructive = desc.includes('DESTRUCTIVE') || desc.includes('80 MPH');
            color = isDestructive ? '#ea580c' : '#ffa500';
            weight = isDestructive ? 3.5 : 2.5;
        } else if (event.includes('Severe Thunderstorm Watch')) {
            color = '#db7093'; weight = 1.5; fillOpacity = 0.25;
        }
